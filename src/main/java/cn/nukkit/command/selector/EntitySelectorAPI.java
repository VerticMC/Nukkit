...
public class EntitySelectorAPI {
    ...
    private static final Cache<String, Boolean> MATCHES_CACHE = CacheBuilder.newBuilder().maximumSize(65535).expireAfterAccess(1, TimeUnit.MINUTES).build();
    ...
    public boolean checkValid(String token) {
        try {
            return MATCHES_CACHE.get(token, () -> ENTITY_SELECTOR.matcher(token).matches());
    ...
    protected List<String> separateArguments(String inputArguments) {
        ...
        for (int i = 0; i < inputArguments.length(); i++) {
            if (inputArguments.charAt(i) == ',' && !goOn) {
