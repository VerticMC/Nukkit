...
public class Scoreboard implements IScoreboard{
    ...
    public boolean removeLine(IScorer scorer) {
        ...
        if (shouldCallEvent()) {
            ScoreboardLineChangeEvent event = new ScoreboardLineChangeEvent(this, removed, removed.getScore(), removed.getScore(), ScoreboardLineChangeEvent.ActionType.REMOVE_LINE);
            Server.getInstance().getPluginManager().callEvent(event);
            if (event.isCancelled()) {
                return false;
            }
    ...
    public boolean removeAllLine(boolean send) {
        ...
        if (shouldCallEvent()) {
            ScoreboardLineChangeEvent event = new ScoreboardLineChangeEvent(this, null, 0, 0, ScoreboardLineChangeEvent.ActionType.REMOVE_ALL_LINES);
            Server.getInstance().getPluginManager().callEvent(event);
            if (event.isCancelled()) {
                return false;
            }
