...
public class ScoreboardManager implements IScoreboardManager{
    ...
    public boolean removeScoreboard(String objectiveName) {
        ...
        ScoreboardObjectiveChangeEvent event = new ScoreboardObjectiveChangeEvent(removed, ScoreboardObjectiveChangeEvent.ActionType.REMOVE);
        Server.getInstance().getPluginManager().callEvent(event);
        if (event.isCancelled()) {
            return false;
        }
    ...
    public void setDisplay(DisplaySlot slot, @Nullable IScoreboard scoreboard) {
        ...
        if (old != null) {
            this.viewers.forEach(viewer -> old.removeViewer(viewer, slot));
        }
        if (scoreboard != null) {
    ...
    public void read() {
        ...
        storage.readDisplay().forEach((slot, objectiveName) -> {
            IScoreboard scoreboard = getScoreboard(objectiveName);
            if (scoreboard != null) {
