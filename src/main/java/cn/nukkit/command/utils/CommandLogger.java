...
public record CommandLogger(Command command,
    ...
    public CommandLogger addSuccess(String key, String... params) {
        if (TextFormat.getLastColors(key).isEmpty()) {
            if (Server.getInstance().getLanguage().internalGet(key) != null) {
    ...
    private void broadcastAdminChannel(String key, String[] value) {
        ...
        if (target instanceof ExecutorCommandSender executorCommandSender) {
            target = executorCommandSender.getExecutor();
        }
        if (target instanceof ICommandBlock) {
            return;
        }
